...
**Use early stopping, not fixed iteration counts.** For tree-based models: use `early_stopping_rounds` (10-20). For neural nets: monitor validation metric per epoch. For Optuna/hyperparameter search: **always set `timeout=` in the study** (e.g., `study.optimize(objective, n_trials=50, timeout=300)`) so it auto-stops if trials are too slow. Never launch an unbounded search — if you don't set a timeout, a single slow trial can block the entire pipeline.
